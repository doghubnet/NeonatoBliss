...
import { MoonStar, HeartHandshake, Baby, CloudMoon, Microscope, CheckCircle } from "lucide-react";
  ...
  return (
    <main>
      <section className="relative min-h-screen overflow-hidden">
        ...
        <div className="section-wrap relative z-10 grid min-h-screen items-center py-16 lg:grid-cols-2">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="font-heading text-4xl leading-tight text-white md:text-6xl">
              Newborn Bliss: Science + Real-Life = Calm Baby & Thriving Growth
            </h1>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Button asChild size="lg"><Link href="/resources#free-guide">Download Free 4-Page Survival Guide</Link></Button>
              <Button asChild variant="outline" size="lg"><Link href="/shop">Browse Resources</Link></Button>
            </div>
            <div className="mt-8 flex flex-wrap gap-2 text-xs text-white/90">
              <Badge>Evidence-Based</Badge><Badge>Instant Digital Delivery</Badge><Badge>30-Day Happiness Guarantee</Badge><Badge>Used Worldwide</Badge>
            </div>
          </motion.div>
          <div className="mx-auto mt-8 w-full max-w-md rounded-3xl bg-white/95 p-6 shadow-soft lg:mt-0">
      ...
      <section className="section-wrap py-16">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {challenges.map((c) => <Card key={c}><p className="font-semibold">{c}</p></Card>)}
        </div>
      </section>

      <section className="section-wrap py-8">
        <h2 className="font-heading text-3xl">How Newborn Bliss Helps</h2>
        <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {[HeartHandshake, MoonStar, Baby, CloudMoon, Microscope, CheckCircle].map((Icon, idx) => (
            <Card key={idx}><Icon className="h-6 w-6 text-terracotta" /><p className="mt-3 text-sm">Responsive Care Lowers Cortisol (Science) + practical routines adaptable for any home (Real Life).</p></Card>
          ))}
        </div>
      </section>

      <section className="section-wrap py-16">
        <h2 className="font-heading text-3xl">Featured Resources / Shop</h2>
          ...
          {products.map((p) => (
            <Card key={p.slug} className="overflow-hidden p-0">
              ...
              <div className="p-5">
                ...
                <ul className="mt-2 list-disc pl-5 text-sm text-navy/80">{p.short.map((b) => <li key={b}>{b}</li>)}</ul>
                <div className="mt-4 flex items-center justify-between"><span className="font-bold">{p.price}</span><Badge>Instant Download</Badge></div>
                <Button asChild className="mt-4 w-full"><Link href={`/product/${p.slug}`}>Learn More</Link></Button>
              </div>
            </Card>
          ))}
        </div>
      ...
      <section className="section-wrap py-16">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {testimonials.map(([name, country, quote]) => (
            <Card key={name}><p className="text-sm">“{quote}”</p><p className="mt-3 text-sm font-semibold">{name} · {country}</p></Card>
          ))}
        </div>
      </section>
      ...
      <section className="section-wrap py-8">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-5">
          {["First-time moms", "Working parents", "Partners", "Adoptive families", "Low-resource homes"].map((a) => <Card key={a} className="text-center text-sm font-semibold">{a}</Card>)}
        </div>
      </section>
      ...
      <section className="section-wrap py-16">
        ...
        <Accordion type="single" collapsible className="mt-6 space-y-3">
          {[
            ...
            "Are dads and partners included?",
            ...
            <AccordionItem key={q} value={`q${i}`}>
              ...
              <AccordionContent>
                Yes. Not medical advice – consult your healthcare provider. Backed by WHO, UNICEF, AAP, and pediatric insights.
              </AccordionContent>
            </AccordionItem>
          ))}
        </Accordion>
      ...
      <section className="relative py-24">
        ...
        <div className="section-wrap relative text-center text-white">
          <h2 className="font-heading text-4xl">Start Your Newborn Bliss Journey Today</h2>
          ...
          <p className="disclaimer mt-5 text-white/90">Not medical advice – consult your healthcare provider. Backed by WHO, UNICEF, AAP, and pediatric insights.</p>
        </div>
      </section>
    </main>
  );
